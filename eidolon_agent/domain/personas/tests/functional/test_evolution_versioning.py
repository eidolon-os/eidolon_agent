"""PersonasService — overlay_version is bumped on every applied evolution.
...
async def test_overlay_version_increments_on_evolve(personas_service) -> None:
    ...
    before = await personas_service.get_instance(
        tenant_id="t", user_id="u", instance_id="i-ver"
    )
    assert before.overlay_version == 1
    ...
    assert after.overlay_version == 2
...
async def test_dry_run_does_not_bump_version(personas_service) -> None:
    ...
    after = await personas_service.get_instance(
        tenant_id="t", user_id="u", instance_id="i-dry"
    )
    assert after.overlay_version == 1
