...
from eidolon_agent.app.runtime.lifecycle import install_shutdown_handlers
...
async def _run(args) -> int:  # type: ignore[no-untyped-def]
    ...
    if not args.http_only:
        ...
        grpc_task = asyncio.create_task(
            container.grpc_server.wait_for_termination(), name="grpc-server"
        )
    ...
    _log.info(
        ...
        settings.grpc.tcp_host,
        settings.grpc.tcp_port,
        settings.http.host,
        settings.http.port,
        settings.http.host,
        settings.http.admin_port,
    )
    ...
    system_data_http = container.extras.get("system_data_http_client")
    if system_data_http is not None and hasattr(system_data_http, "aclose"):
