...
def test_non_key_safe_user_id_is_reversible_base64url_segment() -> None:
...
def test_b64_prefix_is_reserved_to_keep_user_segments_unambiguous() -> None:
    assert safe_user_key("b64_alice").startswith("b64_")
    assert safe_user_key("b64_alice") != "b64_alice"
    assert user_id_from_safe_key(safe_user_key("b64_alice")) == "b64_alice"
