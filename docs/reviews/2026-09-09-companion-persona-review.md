...
| F6 命令与恢复 | 已统一 Data 服务层写入入口；restore/rollback/reset 均追加版本，保留当前名字、用户事实和偏好；历史恢复/改名尚未全部贯通与编辑相同的操作回执和 CAS 协议，不关闭第三批完整退出条件 |
| F7 长期状态归属 | 显式对话偏好已独立存储；旧事实/承诺仍保留，提供只读迁移审查脚本，尚未向真实 Memory 迁移或切断旧读取 |
| F8 创建和编辑 | 两步创建、三个 Data 版本化起点、明确标注的静态示例、独立回复偏好、折叠详细字段已实现；动态隔离试聊未实现，采用原计划允许的首版示例方案 |
| 第四批 | 已将 enabled、证据、身份/关系保护、trait 幅度和 base 校验放到 Data，并与 Agent 共用 SDK 校验；没有开启生产自动演化，仍须长期一致性与跨进程验收 |

因此，本次交付覆盖第一批主要代码、第二批首版，以及第三/四批的前置保护；**没有将四批所有退出条件标为完成**。真实记忆迁移、恢复/改名协议统一和设备实测是后续明确工作项。
...
- 迁移先记录原始字段来源和范围，展示冲突，再停止双读。不得把旧的 Companion 私有设定自动提升为所有伙伴共享事实；关系约定也不能误转为待执行任务。
